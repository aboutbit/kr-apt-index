    ...
    function geocodeApts(items, sigunguNm) {
      return new Promise(function(resolveAll) {
        ...
        kakao.maps.load(function() {
          ...
          var done = 0;
          // 1차: sigunguNm + umdNm + aptNm (더 정확), 실패 시 2차: sigunguNm + aptNm
          function searchApt(apt, query, fallbackQuery, resolve) {
            ps.keywordSearch(query, function(results, status) {
              if (status === kakao.maps.services.Status.OK && results.length > 0) {
                ...
                searchApt(apt, fallbackQuery, null, resolve);
              } else {
                resolve();
              }
            }, { size: 1 });
          ...
          uniqueKeys.forEach(function(key) {
            chain = chain.then(function() {
              var apt = aptKeys[key];
              var q1 = (sigunguNm ? sigunguNm + ' ' : '') + apt.umdNm + ' ' + apt.aptNm;
              var q2 = (sigunguNm ? sigunguNm + ' ' : '') + apt.aptNm;
              return new Promise(function(res) {
                searchApt(apt, q1, q2, function() {
                  done++;
