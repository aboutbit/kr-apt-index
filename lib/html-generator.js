  ...
  <script>
    ...
    async function searchTrade() {
      ...
      try {
        ...
        renderTradeMap(mapItems);

        var mapNote = !useDetailed
          ? ' | 지도: data.go.kr에서 "아파트매매 실거래 상세 자료" 서비스 추가 등록 시 활성화'
          : '';
        showTradeStatus(allItems.length + '건 조회됨 (최근 3개월, 해제 거래 포함)' + mapNote, false);
      } catch(err) {
        console.error(err);
