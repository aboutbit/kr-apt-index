  ...
  <script>
    ...
    function renderTradeMap(items) {
      ...
      Object.values(aptMap).forEach(function(apt) {
        ...
        // 클릭 시 DOM 팝업 패널 표시 (Kakao Maps 외부 div → 항상 마커 위에)
        (function(aptData, elemId) {
          setTimeout(function() {
            var el = document.getElementById(elemId);
            if (el) {
              el.addEventListener('click', function(e) {
                e.stopPropagation();
                showMapPopup(aptData);
              });
            }
          }, 100);
        })(apt, uid);
    ...
    function showMapPopup(apt) {
      var popup = document.getElementById('map-popup');
      var trades = apt.trades || [];
      var rows = trades.slice(0, 5).map(function(it) {
        var raw = parseInt((it.dealAmount || '0').replace(/,/g,''), 10);
        var price = raw >= 10000
          ? (raw / 10000).toFixed(1).replace(/\.0$/, '') + '억'
          : raw.toLocaleString() + '만';
      ...
      var extra = trades.length > 5
        ? '<div style="font-size:0.75em;color:#94a3b8;margin-top:3px;text-align:right">외 ' + (trades.length - 5) + '건</div>'
        : '';
      popup.innerHTML =
        '<div style="position:relative">'
        + '<button onclick="closeMapPopup()" style="position:absolute;top:-4px;right:-4px;background:none;border:none;font-size:18px;cursor:pointer;color:#94a3b8;line-height:1;padding:2px 6px">×</button>'
        + '<div style="font-weight:700;font-size:0.95em;margin-bottom:2px;padding-right:20px">' + apt.aptNm + '</div>'
