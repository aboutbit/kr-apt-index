...
<body>
  ...
  <script>
    ...
    // 아파트별 지오코딩 (Nominatim OpenStreetMap → lat/lng, API 키 불필요)
    // 지번(jibun) 주소 우선 사용 — 아파트명보다 OSM 매칭률이 훨씬 높음
    async function geocodeApts(items, sigunguNm) {
      var aptKeys = {};
      items.forEach(function(it) {
        var key = (it.aptNm || '') + '||' + (it.umdNm || '');
        if (!aptKeys[key]) aptKeys[key] = {
          aptNm: it.aptNm || '', umdNm: it.umdNm || '',
          jibun: it.jibun || '', lat: null, lng: null,
        };
      });
      var uniqueKeys = Object.keys(aptKeys);
      if (uniqueKeys.length === 0) return items;
      // Nominatim: 5건씩 병렬, 배치 간 1초 대기 (rate limit 준수)
      var BATCH = 5;
      for (var i = 0; i < uniqueKeys.length; i += BATCH) {
        var batch = uniqueKeys.slice(i, i + BATCH);
        showTradeStatus('좌표 조회 중... (' + Math.min(i + BATCH, uniqueKeys.length) + '/' + uniqueKeys.length + '개)', false);
        await Promise.all(batch.map(function(key) {
          var apt = aptKeys[key];
          // jibun 있으면 "시군구 법정동 지번" 주소로 검색 (OSM 매칭률↑)
          // 없으면 아파트명 fallback
          var queryStr = apt.jibun
            ? (sigunguNm ? sigunguNm + ' ' : '') + apt.umdNm + ' ' + apt.jibun
            : (sigunguNm ? sigunguNm + ' ' : '') + apt.aptNm + ' ' + apt.umdNm;
          var q = encodeURIComponent(queryStr);
          return fetch('https://nominatim.openstreetmap.org/search?q=' + q + '&format=json&limit=1&countrycodes=kr')
            .then(function(r) { return r.json(); })
            .then(function(data) {
              if (data && data.length > 0) {
                apt.lat = parseFloat(data[0].lat);
                apt.lng = parseFloat(data[0].lon);
              }
            })
            .catch(function() {});
        }));
    ...
    function renderTradeMap(items) {
      ...
      Object.values(aptMap).forEach(function(apt) {
        ...
        var icon = L.divIcon({
          className: '',
          html: '<div style="background:' + color + ';color:#fff;padding:2px 7px;border-radius:12px;'
              + 'font-size:11px;font-weight:700;white-space:nowrap;'
              + 'border:1.5px solid rgba(255,255,255,0.85);box-shadow:0 1px 4px rgba(0,0,0,0.25);'
              + 'cursor:pointer;font-family:-apple-system,sans-serif">' + priceLabel + '</div>',
          iconAnchor: [22, 11],
          iconSize: null,
        });
        var marker = L.marker([apt.lat, apt.lng], { icon: icon });
    ...
    async function searchTrade() {
      ...
      try {
        ...
        var geocodedItems = await geocodeApts(mapItems, sigunguNm);

        var validCoords = geocodedItems.filter(function(it) {
          var la = parseFloat(it.lat), lo = parseFloat(it.lng);
          return !isNaN(la) && !isNaN(lo) && la !== 0 && lo !== 0;
        });
        renderTradeMap(geocodedItems);

        showTradeStatus(allItems.length + '건 조회됨 | 지도: ' + validCoords.length + '/' + uniqueAptCount + '개 아파트 표시', false);
      } catch(err) {
        console.error(err);
