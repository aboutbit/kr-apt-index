  ...
  <script>
        ...
        kakao.maps.load(function() {
          ...
          uniqueKeys.forEach(function(key) {
            chain = chain.then(function() {
              var apt = aptKeys[key];
              var query = (sigunguNm ? sigunguNm + ' ' : '') + apt.aptNm;
    ...
    function renderTradeMap(items) {
      ...
      Object.values(aptMap).forEach(function(apt) {
        ...
        // 가격 라벨 오버레이
        var overlayContent = '<div id="' + uid + '" style="background:' + color
          + ';color:#fff;padding:2px 7px;border-radius:12px;font-size:11px;font-weight:700;'
          + 'white-space:nowrap;border:1.5px solid rgba(255,255,255,0.85);'
          + 'box-shadow:0 1px 4px rgba(0,0,0,0.3);cursor:pointer;'
          + 'font-family:-apple-system,sans-serif">' + priceLabel + '</div>';
