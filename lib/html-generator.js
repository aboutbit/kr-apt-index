  ...
  <script>
    ...
    function geocodeApts(items, sigunguNm) {
      return new Promise(function(resolveAll) {
        ...
        // kakao.maps.load: autoload 여부 무관하게 SDK 완전 초기화 보장
        kakao.maps.load(function() {
          ...
          uniqueKeys.forEach(function(key) {
            chain = chain.then(function() {
              var apt = aptKeys[key];
              var query = (sigunguNm ? sigunguNm + ' ' : '') + apt.aptNm + ' ' + apt.umdNm;
    ...
    async function searchTrade() {
      ...
      try {
        ...
        // 진단: 지오코딩 결과를 상태 표시줄에 표시
        var validCoords = geocodedItems.filter(function(it) {
          var la = parseFloat(it.lat), lo = parseFloat(it.lng);
          return !isNaN(la) && !isNaN(lo) && la !== 0 && lo !== 0;
        });
        var kakaoStatus = typeof kakao === 'undefined' ? '카카오SDK미로드'
          : !kakao.maps ? 'kakao.maps없음'
          : !kakao.maps.services ? 'services없음'
          : (validCoords.length + '/' + uniqueAptCount + '개좌표');
        renderTradeMap(geocodedItems);

        showTradeStatus(allItems.length + '건 조회됨 | 지도진단: ' + kakaoStatus, false);
      } catch(err) {
        console.error(err);
