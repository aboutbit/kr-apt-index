...
async function generateHTML(regionsData, outputPath, molitApiKey = '') {
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    @media (max-width: 640px) {
      .header-left h1 { font-size: 1.25em; }
      .chart-container { height: 260px; }
      .controls { flex-direction: column; align-items: flex-start; }
      .container { padding: 12px; }
      .trade-controls { flex-direction: column; align-items: flex-start; }
    }
  </style>
</head>
<body>
    ...
    function renderTradeTable(dongFilter, aptFilter) {
      ...
      var rows = items.map(function(it) {
        ...
        return '<tr>'
          ...
          + '<td style="color:var(--text-muted);font-size:0.85em">' + (it.dealingGbn || '—') + '</td>'
          + '</tr>';
      }).join('');
      ...
      document.getElementById('trade-table-container').innerHTML =
        ...
        + '<table>'
        + '<thead><tr>'
        + '<th>거래일</th><th>아파트명</th><th>법정동</th><th>전용면적</th><th>층</th>'
        + '<th style="text-align:right">거래금액</th><th>거래유형</th>'
        + '</tr></thead>'
        + '<tbody>' + rows + '</tbody>'
        + '</table></div>';
    }
