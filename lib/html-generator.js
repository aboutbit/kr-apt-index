...
<body>
  ...
  <script>
    ...
    // 아파트별 지오코딩 (Kakao 장소 검색 API → lat/lng)
    function geocodeApts(items, sigunguNm) {
    ...
    function renderTradeMap(items) {
      ...
      _overlayData = [];
      _activeFilter = 'all';
      if (_currentIW) { _currentIW.close(); _currentIW = null; }
      ...
      Object.values(aptMap).forEach(function(apt) {
        ...
        // 가격 라벨 오버레이
        var latestAr = parseFloat(apt.trades[0].excluUseAr || 0);
        var areaText = latestAr > 0
          ? (latestAr.toFixed(1) + '㎡ (' + Math.round(latestAr / 3.3058) + '평)')
          : '';
        var overlayContent = '<div id="' + uid + '" style="'
          ...
          + '<div style="font-size:12px;color:' + color + ';font-weight:700">' + priceLabel + '</div>'
          + (areaText ? '<div style="font-size:9px;color:#64748b;margin-top:1px">' + areaText + '</div>' : '')
          + '</div>';
        ...
        var pyeong = latestAr > 0 ? Math.round(latestAr / 3.3058) : 0;
        overlay.setMap(_kakaoMap);
        _overlays.push(overlay);
        _overlayData.push({ overlay: overlay, pyeong: pyeong });
        ...
        var popupContent = '<div style="min-width:230px;font-family:-apple-system,sans-serif;line-height:1.5;padding:4px 2px">'
          ...
          setTimeout(function() {
            ...
            if (el) {
              el.addEventListener('click', function(e) {
                e.stopPropagation();
                if (_currentIW) _currentIW.close();
                _currentIW = infoWin;
                infoWin.open(_kakaoMap);
              });
            }
          }, 100);
        })(iw, uid);
    ...
    function filterTradeMap(range) {
      ...
      _overlayData.forEach(function(d) {
        var visible = range === 'all'
          || (range === '10' && d.pyeong >= 10 && d.pyeong < 20)
          || (range === '20' && d.pyeong >= 20 && d.pyeong < 30)
          || (range === '30' && d.pyeong >= 30 && d.pyeong < 40)
          || (range === '40' && d.pyeong >= 40);
