    ...
    for (const [sigunguNm, lawdCd] of Object.entries(sigungus)) {
      ...
      for (const ym of months) {
        ...
        for (const it of items) {
          ...
          const clean = aptNm.replace(/\([^)]*\)/g, '').trim();
          const queries = [
            `${sigunguNm} ${umdNm} ${aptNm}`,
            `${sigunguNm} ${aptNm}`,
          ];
          if (clean && clean !== aptNm) {
            queries.push(`${sigunguNm} ${umdNm} ${clean}`);
            queries.push(`${sigunguNm} ${clean}`);
          }

          let found = null;
          for (const q of queries) {
            await sleep(60); // Kakao API 레이트 리밋 방지
            found = await kakaoSearch(q);
            if (found) break;
          }
